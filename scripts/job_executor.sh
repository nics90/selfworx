...
win_execution_user=`grep 'win_execution_user' config/task_config.ini | awk -F '=' '{print $2}'`
win_execution_password=`grep 'win_execution_password' config/task_config.ini | awk -F '=' '{print $2}'`
...
EOF
 ...
 for role in $(ls "roles/$role_name/")
  do
    if [ "$role" = "windows_server_reboot.json" ];then
  ...
  #if [ "$role_name" = "config_win_reboot" ];then
   #  exit_code_on_success=35
   #else
   #  exit_code_on_success=0
   #fi
  knife winrm -m $node_name "chef-client -r 'role[$role_name]' -l info" --winrm-user $win_execution_user --winrm-password $win_execution_password >>$log_dir/$log_file_name
